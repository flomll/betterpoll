...
class MFPollController extends JController
{
	...
	function save()
	{
		...
		foreach ($options as $i=>$text)
		{
			...
			if ($isNew)
			{
				...
				$obj->text   = $text;
				$db->insertObject('#__mfpoll_data', $obj);
			}
			else
			{
				$obj = new stdClass();
				$obj->id     = (int)$i;
				$obj->text   = $text;
				$db->updateObject('#__mfpoll_data', $obj, 'id');
			}
		}
		...
		switch ($this->_task)
		{
			case 'apply':
				...
				$link = 'index.php?option=com_mfpolls&view=poll&task=edit&cid[]='. $row->id .'';
				break;

			case 'save':
			default:
				$msg = JText::_( 'Poll saved' );
				$link = 'index.php?option=com_mfpolls';
				break;
		}
	...
	*/
	function publish()
	{
		...
		$mainframe->redirect( 'index.php?option=com_mfpolls' );
	}
